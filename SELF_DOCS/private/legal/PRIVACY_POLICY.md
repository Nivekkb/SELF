# SELF™ Privacy Policy

**Last Updated:** December 22, 2025  
**Effective Date:** December 22, 2025

## 1. Introduction

SoulsynqAI Inc. ("we", "us", "our") operates the SELF™ (Self-Enforcing Limitless Framework) platform and related services. We are committed to protecting your privacy and handling your personal information responsibly.

This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our SELF™ platform, including our website, API services, and any related products or services.

**Important Note:** Your privacy matters as much as your safety. No personal information or topics of conversation are ever recorded - only that intervention is needed.

## 2. Information We Collect

### 2.1 Information You Provide Directly

**User Registration and Account Information:**
- Name, email address, organization details
- License tier and subscription information
- Payment and billing information
- API keys and authentication credentials

**Usage and Interaction Data:**
- Conversation metadata (timestamps, session duration)
- System usage patterns and feature interactions
- Support requests and communications
- Feedback and survey responses

### 2.2 Information We Collect Automatically

**Technical and Device Information:**
- IP address, browser type, and version
- Operating system and device information
- Cookies and similar tracking technologies
- Usage logs and system performance data

**API Usage Data:**
- API request and response metadata
- Rate limiting and usage statistics
- Error logs and system diagnostics

### 2.3 Information from Third Parties

- Information from payment processors
- Data from identity verification services
- Analytics and usage data from third-party services

## 3. How We Use Your Information

We use the information we collect for the following purposes:

### 3.1 Service Delivery and Operation
- Provide, maintain, and improve our SELF™ services
- Process transactions and manage subscriptions
- Authenticate users and API access
- Monitor system performance and security

### 3.2 Safety and Compliance
- Detect and prevent safety violations
- Monitor for abuse and misuse of services
- Comply with legal and regulatory requirements
- Conduct security investigations and risk assessments

### 3.3 Communication and Support
- Respond to user inquiries and support requests
- Send service notifications and updates
- Provide customer support and troubleshooting
- Communicate about changes to our services

### 3.4 Analytics and Improvement
- Analyze usage patterns and service performance
- Develop new features and improve existing ones
- Conduct research and development
- Generate usage statistics and reports

## 4. Information Sharing and Disclosure

We do not sell, trade, or otherwise transfer your personal information to third parties, except as described below:

### 4.1 Service Providers
We may share information with third-party service providers who perform services on our behalf, including:
- Payment processing and billing
- Data analysis and analytics
- Customer support and communication
- Security and fraud prevention
- Cloud hosting and infrastructure

### 4.2 Legal and Regulatory Requirements
We may disclose your information when required by law or in response to:
- Legal process (subpoenas, court orders, etc.)
- Law enforcement requests
- Regulatory requirements
- To protect our rights, privacy, safety, or property

### 4.3 Business Transfers
In connection with any merger, sale of company assets, financing, or acquisition, user information may be transferred as part of the transaction.

### 4.4 With Your Consent
We may share your information for any other purpose disclosed to you and with your consent.

## 5. Data Retention and Security

### 5.1 Data Retention
- **Session Data:** Retained only during active conversations for safety context
- **Usage Logs:** Retained for 90 days for operational and security purposes
- **Account Information:** Retained for the duration of your account and as required by law
- **Compliance Records:** Retained as required by applicable regulations

### 5.2 Security Measures
We implement industry-standard security measures to protect your information:
- **Encryption:** Data encrypted in transit and at rest
- **Access Controls:** Role-based access and authentication
- **Security Monitoring:** Continuous monitoring for threats and vulnerabilities
- **Incident Response:** Procedures for responding to data breaches

### 5.3 Data Minimization
We collect only the minimum information necessary for service operation and safety compliance.

## 6. Your Privacy Rights and Choices

### 6.1 Access and Control
You have the right to:
- Access and obtain a copy of your personal information
- Request correction of inaccurate information
- Request deletion of your personal information
- Object to or restrict processing of your information
- Data portability (where applicable)

### 6.2 Cookies and Tracking
You can control cookies and tracking technologies through your browser settings and our cookie preferences center.

### 6.3 Marketing Communications
You can opt out of marketing communications by following the unsubscribe instructions in our emails.

### 6.4 Do Not Sell
We do not sell your personal information. No opt-out is required.

## 7. International Data Transfers

Your information may be transferred to and processed in countries other than your own, including Canada, where we are headquartered. We ensure appropriate safeguards are in place for international data transfers.

## 8. Children's Privacy

Our services are not intended for individuals under 18 years of age. We do not knowingly collect personal information from children.

## 9. Privacy Officer and Contact Information

**Privacy Officer Contact:**
- Email: privacy@SoulsynqAI.com
- Address: SoulsynqAI Inc., [Company Address]
- Phone: [Company Phone Number]

**Data Protection Officer (where applicable):**
- Email: dpo@SoulsynqAI.com

## 10. Your Rights by Jurisdiction

### 10.1 California Residents (CCPA/CPRA)
California residents have additional rights under the California Consumer Privacy Act:
- Right to know what personal information is collected and used
- Right to delete personal information
- Right to opt out of sale of personal information (not applicable - we don't sell data)
- Right to non-discrimination for exercising privacy rights

### 10.2 European Union/EEA Residents (GDPR)
EU/EEA residents have rights under the General Data Protection Regulation:
- Right to access, rectification, erasure, restriction, and portability
- Right to object to processing
- Right to lodge a complaint with a supervisory authority

### 10.3 Canadian Residents (PIPEDA)
Canadian residents have rights under the Personal Information Protection and Electronic Documents Act:
- Right to access and correct personal information
- Right to know how information is used and disclosed
- Right to withdraw consent

## 11. Data Breach Notification

In the event of a data breach affecting your personal information, we will:
- Notify you without unreasonable delay
- Provide information about the breach and its impact
- Advise on steps to protect yourself
- Cooperate with regulatory authorities as required

## 12. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of any changes by:
- Posting the updated policy on our website
- Sending notification via email or other communication
- Posting notice in our application

**Your Continued Use:** Your continued use of our services after any changes indicates your acceptance of the updated policy.

## 13. Third-Party Links

Our services may contain links to third-party websites or services. This Privacy Policy applies only to our services. We are not responsible for the privacy practices of third-party services.

## 14. Legal Basis for Processing (GDPR)

For EU/EEA residents, we process your personal information based on the following legal bases:
- **Contract Performance:** To provide services you have requested
- **Legitimate Interests:** For service improvement, security, and fraud prevention
- **Legal Obligations:** To comply with applicable laws and regulations
- **Consent:** Where we have obtained your explicit consent

## 15. Cross-Border Data Flow

### 15.1 Data Flow Architecture
- **Collection:** Data collected at point of interaction
- **Processing:** Centralized processing in secure data centers
- **Storage:** Encrypted storage with access controls
- **Transfer:** Secure transfer protocols with encryption

### 15.2 International Standards
We comply with international data protection standards and frameworks, including:
- ISO/IEC 27001: Information Security Management
- NIST Cybersecurity Framework
- Cloud Security Alliance (CSA) guidelines

## 16. Accountability and Governance

### 16.1 Privacy by Design
We implement privacy principles throughout our service lifecycle:
- Privacy impact assessments for new features
- Data protection by design and by default
- Regular privacy training for employees

### 16.2 Compliance Monitoring
- Regular audits of privacy practices
- Third-party security assessments
- Compliance with industry standards

## 17. Definitions

- **Personal Information:** Any information that identifies or can be used to identify an individual
- **Processing:** Any operation performed on personal information
- **Controller:** Entity that determines the purposes and means of processing personal information
- **Processor:** Entity that processes personal information on behalf of the controller

## 18. Glossary

- **API:** Application Programming Interface
- **GDPR:** General Data Protection Regulation (EU)
- **CCPA:** California Consumer Privacy Act
- **PIPEDA:** Personal Information Protection and Electronic Documents Act (Canada)
- **Encryption:** Process of converting information into unreadable format
- **Anonymization:** Process of irreversibly removing personal identifiers

## 19. Additional Provisions

### 19.1 Limitation of Liability
To the extent permitted by law, we are not liable for any indirect, incidental, special, consequential, or punitive damages arising from privacy-related issues.

### 19.2 Indemnification
You agree to indemnify and hold us harmless from any claims arising from your violation of this Privacy Policy.

### 19.3 Severability
If any provision of this Privacy Policy is found to be invalid, the remaining provisions remain in full force and effect.

## 20. Contact Us

If you have questions about this Privacy Policy or our privacy practices, please contact us:

**SoulsynqAI Inc.**
Privacy Officer
Email: privacy@SoulsynqAI.com
Website: [Company Website]
Address: [Company Address]

**For Data Protection Inquiries:**
Data Protection Officer
Email: dpo@SoulsynqAI.com

We are committed to resolving privacy concerns promptly and fairly. If you believe your privacy rights have been violated, we encourage you to contact us first. You also have the right to lodge a complaint with your local data protection authority.

---

**Effective Date:** December 22, 2025  
**Last Updated:** December 22, 2025

*This Privacy Policy is part of our comprehensive legal framework. Please also review our Terms of Service and other legal documents for complete information about your rights and responsibilities when using our services.*
