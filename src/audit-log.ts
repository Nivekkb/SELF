...
export function toOpaqueAuditId(label: "user" | "session", raw: string): string {
...
function resolveAuditLogPath(explicit?: string): string {
  if (explicit) return explicit;
  if (process.env.SELF_AUDIT_LOG_PATH) return process.env.SELF_AUDIT_LOG_PATH;
  return "";
}

export function logSelfAuditEvent(
  event: SelfAuditEvent,
