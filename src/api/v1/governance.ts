...
import { getGovernanceConfigManager, GovernanceConfig } from './governance-config';

// Interfaces from the requirements
interface GovernanceRequest {
  // Basic request information
  requestId?: string; // SELF will generate if not provided
  projectId: string;
  userId: string;
  ...
  // Enterprise subdomain (for routing)
...
const governanceRequests: Record<string, GovernanceResponse> = {};
const governanceAuditLogs: Record<string, GovernanceStatusResponse['auditLog']> = {};
...
async function authenticateRequest(req: Request): Promise<{ userId: string; email: string; role: string }> {
...
function validateBuildConfig(buildConfig: GovernanceRequest['buildConfig']): { valid: boolean; errors?: string[] } {
  const errors: string[] = [];
...
function validateUserPermissions(userRole: string, requestType: GovernanceRequest['requestType']): boolean {
  // Only admins and developers can submit requests
  return ['admin', 'developer'].includes(userRole);
}

// Approval workflow functions
function determineRequiredApprovals(environment: GovernanceRequest['environment'], requestType: GovernanceRequest['requestType']): string[] {
...
function createGovernanceResponse(request: GovernanceRequest, requestId: string): GovernanceResponse {
...
async function runComplianceScan(request: GovernanceRequest): Promise<GovernanceResponse['complianceCheck']> {
...
async function runSecurityScan(request: GovernanceRequest): Promise<GovernanceResponse['securityScan']> {
...
async function runBudgetApproval(request: GovernanceRequest): Promise<GovernanceResponse['budgetApproval']> {
...
async function handleGovernanceRequest(request: GovernanceRequest): Promise<GovernanceResponse> {
  ...
  if (!configValidation.valid) {
    const errorResponse: GovernanceResponse = {
      id: requestId,
      status: 'REJECTED',
      createdAt: now,
      updatedAt: now,
...
export function createGovernanceRouter(): Router {
  ...
  router.post('/requests', async (req: Request, res: Response) => {
    try {
      ...
      const requestData: GovernanceRequest = req.body;
      ...
      const missingFields = requiredFields.filter(field => !requestData[field as keyof GovernanceRequest]);
      ...
      const errorResponse: GovernanceError = {
