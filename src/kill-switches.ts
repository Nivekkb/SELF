...
export function recordKillSwitchState(
...
export function checkUnsafeResumeAfterDistress(
  context: KillSwitchContext,
  ...
  if (hasStabilizationSignals) return false;
  ...
  const distressReappeared = recentHistory.some(msg =>
    msg.role === "user" && detectDistressInMessage(msg.content)
  );

  if (distressReappeared) {
    context.killSwitchState.unsafeResumeDetected = true;
    context.killSwitchState.killSwitchReasons.push(
      `Unsafe resume after distress: S1+→S0 transition without stabilization, distress reappeared within ${context.config.unsafeResumeThreshold} turns`
    );
...
export function checkColdStartMisclassification(
  context: KillSwitchContext,
  currentState: EmotionalState,
  ...
  if (!context.isColdStart) return false;
  ...
  if (misclassificationExceeded || highConfidenceDuringColdStart) {
    ...
    context.killSwitchState.killSwitchReasons.push(
      `Cold-start misclassification: rate ${coldStartMisclassificationRate}% > ${context.config.coldStartMisclassificationThreshold}% threshold ` +
...
export function checkUnloggedDecisions(
  context: KillSwitchContext,
  ...
  if (hasMissingFields) {
    ...
    if (missingConfidence) missingFields.push("confidence");
    if (missingUncertainty) missingFields.push("uncertaintyReasons");
    if (missingConsideredActions) missingFields.push("consideredActions");
    if (missingBlockedActions) missingFields.push("blockedActions");

    context.killSwitchState.unloggedDecisionsDetected = true;
    context.killSwitchState.killSwitchReasons.push(
      `Unlogged decisions: state transition occurred without required fields: ${missingFields.join(", ")}`
    );
    context.killSwitchState.lockedToGuardedState = true;
    context.killSwitchState.requireManualReview = true;
    return true;
...
export function checkHumanReviewFailure(
  context: KillSwitchContext,
  humanReviewFlags: string[]
): boolean {
  // Check if any human review flags are present
  const hasSevereFailure = humanReviewFlags.some(flag =>
    context.config.humanReviewFlags.includes(flag)
  );

  if (hasSevereFailure) {
    context.killSwitchState.humanReviewFailureDetected = true;
    context.killSwitchState.killSwitchReasons.push(
      `Severe failure flagged by human review: ${humanReviewFlags.join(", ")}`
    );
...
export function checkAbuseRecoveryPatterns(
  context: KillSwitchContext,
  message: string,
  ...
  const hasRecoveryAttemptPattern = recoveryAttemptPatterns.some(pattern =>
  ...
  const hasExitSeekingLanguage = exitSeekingPatterns.some(pattern =>
    message.toLowerCase().includes(pattern)
  );
  ...
  if (context.abuseRecoveryAttempts >= 3) {
    ...
    context.killSwitchState.killSwitchReasons.push(
      `Abuse/gaming detected in recovery logic: ${context.abuseRecoveryAttempts} recovery attempts detected`
    );
    context.killSwitchState.lockedToGuardedState = true;
    context.killSwitchState.requireManualReview = true;
    return true;
...
function detectAbusePatternsInContext(context: AbusePreventionContext): { isAbuseDetected: boolean; reasons: string[] } {
...
function checkForStabilizationSignals(message: string): boolean {
...
function detectDistressInMessage(message: string): boolean {
  const normalized = message.toLowerCase();
...
export function checkAllKillSwitches(
  ...
  const unsafeResume = checkUnsafeResumeAfterDistress(
    context,
    args.currentState,
    args.previousState,
    args.message,
  ...
  const coldStartMisclassification = checkColdStartMisclassification(
    context,
    args.currentState,
    args.detectionResult,
    args.coldStartMisclassificationRate,
    args.confidence
  );

  const unloggedDecisions = checkUnloggedDecisions(
    context,
    args.stateTransitionOccurred,
    args.metadata
  );

  const humanReviewFailure = checkHumanReviewFailure(
    context,
    args.humanReviewFlags
  );

  const abuseRecovery = checkAbuseRecoveryPatterns(
    context,
    args.message,
    args.abuseContext
  );
...
export function applyKillSwitchActions(
  context: KillSwitchContext,
  currentState: EmotionalState
): EmotionalState {
...
export function serializeKillSwitchContext(context: KillSwitchContext): string {
...
export function deserializeKillSwitchContext(serialized: string): KillSwitchContext {
  try {
    const data = JSON.parse(serialized);
    return {
      userId: data.userId,
      sessionId: data.sessionId,
      stateHistory: data.stateHistory.map((state: any) => ({
        state: state.state,
