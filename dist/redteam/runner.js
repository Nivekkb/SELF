...
import { REDTEAM_TEST_CASES, getCriticalTestCases, getHighSeverityTestCases } from './test-cases.js';
import { DEFAULT_REDTEAM_CONFIG } from './types.js';
 ...
 */
const normalizeTriggerName = (trigger) => {
    const normalized = trigger.toUpperCase().replace(/\s+/g, '_');
    // Handle common variations
    if (normalized === 'SELFHARM')
        return 'SELF_HARM';
    if (normalized === 'IMPLIED_SELFHARM')
        return 'IMPLIED_SELF_HARM';
    if (normalized === 'HARM_OTHERS' || normalized === 'HARMOTHERS')
        return 'HARM_OTHERS';
    if (normalized === 'ANGRY_PHYSICALITY' || normalized === 'ANGRYPHYSICALITY')
        return 'ANGRY_PHYSICALITY';
    return normalized;
};
/**
 * Check if expected triggers are present in actual triggers
 */
const checkTriggers = (expectedTriggers, actualTriggers, config) => {
 ...
 */
const checkScores = (expectedScores, actualScores, tolerance) => {
...
export function runTest(testCase, config = {}) {
    ...
    // ============================================
    const { missing: missingTriggers, unexpected: unexpectedTriggers } = checkTriggers(testCase.expectedTriggers, detection.triggers || [], fullConfig);
    ...
    // 3. SCORE ASSERTIONS
    // ============================================
    const scoreCheck = checkScores(testCase.expectedScores, detection.scores, fullConfig.scoreTolerance);
 ...
 */
const computeCategoryResults = (results, testCases) => {
 ...
 */
const filterCriticalIssues = (results, testCases) => {
...
export function runHighSeverityTests() {
