...
function normalizeUserInputForGovernance(text) {
    const corrections = [];
    let out = String(text || "");
    try {
...
function detectHarmOthersIntent(message) {
...
function detectSmallTalkIntent(text) {
    ...
    return (/^(hi|hello|hey|yo|sup|good (morning|afternoon|evening))\b/i.test(t) ||
        /\bhow are you\b/i.test(t) ||
        /\bwhat'?s up\b/i.test(t));
}
function detectAcknowledgementIntent(text) {
    const t = normalizeUserInputForGovernance(text).text;
    return (/^(ok|okay|alright|thanks|thank you|ty|got it|cool|makes sense|that makes sense|sounds good)\b/i.test(t) ||
        /^(ok|okay|alright)\s*\.*$/i.test(t));
}
function detectClinicalAuthorityRequest(text) {
    const t = normalizeUserInputForGovernance(text).text;
    return (/\b(can you|could you|will you)\s+(diagnose|diagnose me|tell me what disorder)\b/i.test(t) ||
...
function detectTherapyRoleRequest(text) {
    const t = normalizeUserInputForGovernance(text).text;
    return (/\b(be|act as)\s+(my\s+)?therapist\b/i.test(t) ||
...
export function applyPolicyToPrompt(policy, baseSystemPrompt, variant) {
    ...
    if (policy.requiresGovernanceFallback) {
        lines.push(`Include this safe clarification line: "${policy.governanceFallbackLine || selfConfig.lexicon.confusionFallbackLine}"`);
    }
...
function detectSelfHarmInstructionLanguage(output) {
...
function detectClinicalAuthorityClaim(output) {
    const text = String(output || "");
    const matches = [];
    const patterns = [
...
function detectDiagnosisLanguage(output) {
    const text = String(output || "");
    const matches = [];
    const patterns = [
...
function detectDependencyDynamics(output) {
    const text = String(output || "");
    const matches = [];
    const patterns = [
...
function detectOverrideAttemptLanguage(output) {
    const text = String(output || "");
    const matches = [];
    const patterns = [
...
function detectMedicalAdviceLanguage(output) {
    const text = String(output || "");
    const low = normalize(text);
    ...
    const negationWindow = /\b(can't|cannot|won't|will not|not able to|unable to|i'm not|im not|i am not)\b.{0,60}\b(prescribe|dosage|mg\b|ssri|benzodiazepine|antidepressant|xanax|prozac|sertraline|fluoxetine|escitalopram)\b/i.test(text);
    if (negationWindow)
        return { detected: false, matches: [] };
...
function hasCrisisCueNormalized(low) {
    if (!low)
        return false;
    return (/\b988\b/.test(low) ||
...
export function validateOutput(output, policy) {
    ...
    if (policy.requiresGovernanceFallback) {
        ...
        const has = line ? normalizeForMatch(output).includes(normalizeForMatch(line)) : true;
        if (!has)
            violations.push("Missing safe clarification (governance fallback) line");
    }
...
export function repairOutput(output, policy) {
    ...
    if (needsHardRepair) {
        ...
        if (overrideAttempt.detected)
            safe.push(base[1]);
        if (dependency.detected)
            safe.push("You deserve real-world support too — if you can, consider reaching out to someone you trust.");
        if (medicalAdvice.detected)
            safe.push("For medical questions, a licensed clinician is the right person to ask.");
        safe.push(base[2]);
    ...
    if (policy.maxQuestions > 0) {
        ...
        result = result.replace(sentencePattern, (full, lead, sentence, dot) => {
            if (currentQuestions >= policy.maxQuestions)
                return full;
            const trimmed = String(sentence || "").trim();
            if (!trimmed)
                return full;
            if (!questionLike.test(trimmed))
                return full;
            currentQuestions += 1;
            return `${lead}${trimmed}?`;
        });
    ...
    if (finalWords.length > policy.maxWords) {
        ...
        const tailEstimateWords = `${crisisTail} ${groundingTail} ${agencyTail}`.trim().split(/\s+/).filter(Boolean).length + 2;
