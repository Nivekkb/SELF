...
Respond with a JSON object containing:
{
  ...
  "triggers": ["list of trigger categories detected"]
    ...
    parseResponse(content, modelId) {
        try {
            const parsed = JSON.parse(content);
            // Validate and normalize state
            const validStates = ['S0', 'S0_GUARDED', 'S1', 'S2', 'S3'];
...
export class ModelBasedDetector {
    ...
    async detectState(message, history, context) {
        ...
        for (const provider of this.providers) {
            try {
                const isAvailable = await this.withTimeout(provider.isAvailable(), this.config.timeout);
                if (!isAvailable) {
                    continue;
                }
                const result = await this.withTimeout(provider.classify(message, {
                    ...context,
     ...
     */
    fallbackToLexicon(message, reason, history) {
 ...
 */
export function getModelDetector() {
    if (!detectorInstance) {
 ...
 */
export function resetModelDetector() {
    detectorInstance = null;
}
/**
 * Convenience function for async state detection
 */
export async function detectStateWithModel(message, history, context) {
