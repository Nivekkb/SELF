...
import { adjustPolicyForVariant, applyPolicyToPrompt, applySocialPolicyOverrides, applyStateGatedResponseContract, advanceStickySelfState, detectState, defaultStickySelfSessionState, generateClarifierQuestion, getEffectivePolicy, getS1Variant, getS2Variant, logSelfEvent, maybeAddFollowUpQuestion, processAngerPhysicalityClarifier, repairOutput, rewriteContinuityQuestions, rewriteSpokenMemoryRecall, validateOutput, } from "../index.js";
...
function hmacSha256Base64Url(key, input) {
...
const demoReportRate = new Map(); // clientHash -> epochMs timestamps
...
function isAcceptableDisplayName(name) {
    ...
    if (!n)
        return false;
    if (/\bhttps?:\/\//i.test(n) || /\bwww\./i.test(n))
        return false;
    if (/@/.test(n))
        return false;
...
function allowWeeklyReport(clientHash) {
    const now = Date.now();
...
function isSafeScreenshotPayload(payload) {
    if (!payload || typeof payload !== "object")
        return false;
    const mime = String(payload.mime || "");
    const dataUrl = String(payload.dataUrl || "");
    const bytes = Number(payload.bytes || 0);
    if (!mime || !dataUrl || !Number.isFinite(bytes) || bytes <= 0)
        return false;
...
function resolveSelfSiteLogPath() {
    const configured = String(process.env.SELF_SITE_LOG_PATH || "").trim();
    if (configured)
        return configured;
    const home = process.env.HOME || "";
    if (home)
        return path.join(home, "self-site-flags.jsonl");
    return "";
}
...
function appendSelfSiteLog(event) {
    try {
        console.log(`[SELF_SITE_FLAG] ${JSON.stringify(event)}`);
        if (!SELF_SITE_LOG_PATH)
            return;
...
function metricsSnapshot() {
    const byState = {};
    for (const [k, v] of demoFlagMetrics.byState.entries())
        byState[k] = v;
    const byReason = {};
    for (const [k, v] of demoFlagMetrics.byReason.entries())
        byReason[k] = v;
    return {
...
function getDemoSessionKey(body, req, token) {
    const sid = typeof body.sessionId === "string" ? body.sessionId.trim() : "";
    if (sid)
        return `sid:${sid.slice(0, 80)}`;
    return `client:${getClientId(req, token)}`;
}
function pruneDemoSessions(max) {
    if (demoSessionStates.size <= max)
        return;
...
function detectDemoBoundaryTriggers(userMessage) {
...
function detectDemoUnsafeCategories(output) {
    const text = String(output || "");
    const cats = new Set();
    const low = text.toLowerCase();
    if (/\b(as your (doctor|therapist|psychiatrist|counselor)|i am (a|your) (doctor|therapist|psychiatrist|counselor)|licensed (therapist|psychiatrist|clinician))\b/i.test(text)) {
    ...
    if (/\b(prescribe|dosage|mg\b|ssri|benzodiazepine|antidepressant|xanax|prozac|sertraline|fluoxetine|escitalopram)\b/i.test(text)) {
        // Avoid false positives for refusals/boundary-setting ("I can't prescribe...").
        const negationWindow = /\b(can't|cannot|won't|will not|not able to|unable to|i'm not|im not|i am not)\b.{0,50}\b(prescribe|dosage|mg\b|ssri|benzodiazepine|antidepressant|xanax|prozac|sertraline|fluoxetine|escitalopram)\b/i.test(text);
        const handoffContext = /\b(talk to|see|contact|reach out to)\s+(a|your)\s+(doctor|clinician|physician|provider|psychiatrist)\b/i.test(text) ||
        ...
        const directiveContext = /\b(you should|try|start|begin|take|increase|decrease|reduce|taper|switch|use)\b/i.test(text) ||
    ...
    const instructionSignals = /\b(how to|steps to|here'?s how|method|plan|instructions|you should|do this)\b/i.test(text) ||
...
function blockIfUnsafe(label, output) {
    ...
    const unsafeCategories = label === "governed"
    ...
    if (unsafeCategories.length === 0)
        return { output, blocked: false, unsafeCategories };
    const safeMessage = label === "baseline"
...
function httpsJsonRequest(url, body, headers) {
...
async function generateWithGroq(args) {
    ...
    if (status < 200 || status >= 300) {
        ...
        error.details = typeof json === "object" && json ? json : raw?.slice(0, 500);
        throw error;
    }
...
function buildDemoDraftResponse(args) {
    const { userMessage, state, triggers } = args;
    const intro = state === "S3"
...
function buildDemoGovernedResponse(args) {
    const { userMessage, state, triggers } = args;
    const boundary = triggers.therapistRoleplay || triggers.diagnosisRequest
        ...
        : triggers.authorityPressure
            ...
            : "";
    const grounding = state === "S3" || state === "S2" || state === "S1"
        ? "If it helps, take one slow breath and feel your feet on the ground."
        : "";
    const validation = state === "S3"
    ...
    const crisis = state === "S3"
        ...
        : "";
    const question = state === "S1" ? "Do you want support, or do you want practical options?" : state === "S2" || state === "S3" ? "Are you alone right now?" : "";
    return [boundary, validation, grounding, agencyStep, crisis, question].filter(Boolean).join(" ").trim();
}
const DEMO_BASELINE_SYSTEM_PROMPT = "You are a helpful assistant. Respond normally and directly to the user's message.";
const DEMO_GOVERNED_BASE_SYSTEM_PROMPT = "You are a helpful, calm, non-directive assistant in a governance demo. Do not provide medical advice or diagnosis. Do not claim to be a therapist/doctor. Do not create dependency dynamics. Maintain steady, warm tone. Keep it useful and grounded.";
    ...
    const server = createServer(async (req, res) => {
        try {
            ...
            if (req.method === "POST" && url.pathname === "/v1/demo/chat") {
                ...
                const seed = typeof body.seed === "string"
                    ...
                    : typeof body.sessionId === "string" || typeof body.userId === "string"
                        ? `${String(body.userId || "")}:${String(body.sessionId || "")}`.trim()
                        : undefined;
                const variant = typeof body.variant === "string" ? body.variant : pickVariant(effectiveState, { seed });
                ...
                const governedSystemPrompt = applyPolicyToPrompt(policy, DEMO_GOVERNED_BASE_SYSTEM_PROMPT, variant) || DEMO_GOVERNED_BASE_SYSTEM_PROMPT;
                const groqApiKeyPresent = Boolean(String(process.env.GROQ_API_KEY || process.env.SELF_DEMO_GROQ_API_KEY || "").trim());
                ...
                const demoMode = (typeof body.mode === "string" ? body.mode : "compare");
                ...
                if (triggers.therapistRoleplay || triggers.diagnosisRequest)
                    auditChecks.push("boundary_no_clinical_authority");
                auditChecks.push("tone_lock_calm_warm_nondirective");
                if (governedRepaired)
                    auditChecks.push("repair_applied");
                if (baselinePost.violations.some((v) => v.startsWith("Word count")))
                    auditChecks.push("word_cap_enforced");
                if (baselinePost.violations.some((v) => v.startsWith("Question count")))
                    auditChecks.push("question_cap_enforced");
            ...
            if (req.method === "POST" && url.pathname === "/v1/demo/flag") {
                ...
                if (includePublic)
                    demoFlagMetrics.totalPublicOptIn += 1;
            ...
            if (req.method === "POST" && url.pathname === "/v1/demo/report") {
                ...
                const tier = tierNum === 3 ? 3 : tierNum === 2 ? 2 : 1;
                const why = typeof body.why === "string" ? body.why.trim() : "";
                const prompt = typeof body.prompt === "string" ? body.prompt : "";
                const governedResponse = typeof body.governedResponse === "string" ? body.governedResponse : "";
                const baselineResponse = typeof body.baselineResponse === "string" ? body.baselineResponse : "";
                const consentLeaderboard = Boolean(body.consentLeaderboard);
